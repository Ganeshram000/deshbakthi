...
const ORIGIN = 'https://indiantruckmusic.codewale.in';
...
// Module-level flag so script is only injected once across HMR/StrictMode cycles
...
export default function MusicPlayer({ playlistId = PLAYLIST_ID }) {
  ...
  function pollForIds(player, attempt) {
    ...
    if (attempt < 40) {
      pollRef.current = setTimeout(() => pollForIds(player, attempt + 1), 500);
    }
  }
  ...
  function onStateChange(e) {
    ...
    if (e.data === S.PLAYING) {
      setPlaying(true); syncTrack(e.target);
      clearInterval(timerRef.current);
  ...
  const togglePlay = () => {
    try {
      ...
      else { playerRef.current?.unMute(); playerRef.current?.setVolume(volume); playerRef.current?.playVideo(); }
    } catch (_) {}
  };
  ...
  const handleTrackClick = (idx) => { try { playerRef.current?.playVideoAt(idx); } catch (_) {} setShowPlaylist(false); };
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-50 w-[min(480px,calc(100vw-24px))]">
        ...
        <div className="relative flex flex-col gap-1.5 px-3 py-2.5 rounded-[20px]"
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handlePrev} className="p-1 text-base active:scale-90" style={{ color: 'rgba(0,0,0,0.6)' }}><IoPlaySkipBack /></button>
            ...
            <button onClick={handleNext} className="p-1 text-base active:scale-90" style={{ color: 'rgba(0,0,0,0.6)' }}><IoPlaySkipForward /></button>
        ...
        <div className="absolute inset-0 z-[200] flex items-center justify-center bg-white/20"
          ...
          <div className="relative flex flex-col w-[min(580px,calc(100vw-32px))] max-h-[75vh] overflow-hidden"
            style={{
              ...
              border: '1px solid rgba(255,255,255,0.35)',
              borderRadius: '30px', boxShadow: '0 8px 30px rgba(0,0,0,0.2)',
            }}
